...
VehicleData ThrDeadzoneCalibScreen::update(int rawX, int rawY) {
    ...
    if (yUp && (now - _yUpStart >= CONFIRM_MS)) {
        ...
        debugLogger.logf("[THR DEADZONE] Set: #define JOY_DEADZONE %d", _deadzone);
        showConfirmed();
        return {2048, 2048};
    }
...
void ThrDeadzoneCalibScreen::showAdjusting(int rawY) {
    ...
    d->displayCalibrationStep("THROTTLE DEADZONE", 1, 1, inst,
...
void ThrDeadzoneCalibScreen::showConfirmed() {
    ...
    if (!d) return;
    char l1[28];
    snprintf(l1, sizeof(l1), "JOY_DEADZONE = %d", _deadzone);
    d->displayCalibrationResult("THROTTLE DEADZONE", l1, "Edit ControlConfig.h", "then reflash.");
}
